...
inline void checkCudaErrors(const char * message = "") {
    ...
    if (cudaSuccess != err) {
        fprintf(stderr, "CUDA error '%s' (%d): %s.\n", message, (int)err, cudaGetErrorString(err));
        exit(1);
    }
}
...
bool encode_image(const char * input_filename) {
    ...
    defer { free(block_data); };

#if __CUDACC__
    void *d_rgba_block_data = NULL;
    cudaMalloc(&d_rgba_block_data, block_count * 4 * 4 * 4);
    defer { cudaFree(d_rgba_block_data); };

    void *d_block_data = NULL;
    cudaMalloc(&d_block_data, block_count * 8);
    defer { cudaFree(d_block_data); };

    cudaMemcpy(d_rgba_block_data, rgba_block_data, block_count * 4 * 4 * 4, cudaMemcpyHostToDevice);    
    cudaDeviceSynchronize();  // @@ Is this necessary?    

    checkCudaErrors("Upload");
#endif
    ...
    for (int i = 0; i < repeat_count; i++) {
        ...
        compress_dxt1<<<block_count, NUM_THREADS>>>((uint*)d_rgba_block_data, (uint2 *)d_block_data, 0);

        //cudaMemcpy(block_data, d_block_data, block_count * 8, cudaMemcpyDeviceToHost);

        cudaDeviceSynchronize();  // @@ Is this necessary?
        
        checkCudaErrors("Launch");
    #else
...
int main(int argc, char * argv[]) {
    ...
    if (computeMode == cudaComputeModeProhibited) {
        printf("device is running in <Compute Mode Prohibited>.\n");
        return 0;
    }
    ...
    // getArchName(major, minor)
    // @@ What about Fermi and below?
    // {0x30, "Kepler"},
    // {0x32, "Kepler"},
    // {0x35, "Kepler"},
    // {0x37, "Kepler"},
    ...
    printf("Using CUDA Device %d.%d with %d cores.\n", major, minor, multiProcessorCount);

    checkCudaErrors("Initialization");

    // @@ Detect cuda? select device?
    compute_permutations();

    checkCudaErrors("Compute permutations");
#else
